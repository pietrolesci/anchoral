...
class ClassificationDataModule(DataModule):
    ...
    def __init__(
        self, tokenizer: Optional[PreTrainedTokenizerBase], max_source_length: int = 128, **kwargs
    ) -> None:
    ...
    def from_dataset_dict(
        cls, dataset_dict: DatasetDict, tokenizer: PreTrainedTokenizerBase, columns_to_keep: Optional[List[str]] = None, columns_on_cpu: Optional[List[str]] = None, **kwargs
    ) -> None:
        columns_to_keep = columns_to_keep or []
        columns_to_keep += cls._default_columns
        
        columns_on_cpu = columns_on_cpu or []
        assert all(col in columns_to_keep for col in columns_on_cpu), "`columns_on_cpu` must be a subset of `columns_to_keep`"
