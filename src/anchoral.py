...
class AnchorAL(BaseSubsetWithSearchStrategy):
    ...
    def select_search_query(
        ...
        anchor_ids = {}
        classes = sorted(train_df[InputKeys.TARGET].unique().tolist())  # sort to avoid any randomness
        for c in classes:

            anchor_strategy = self.anchor_strategy_minority if c in self.minority_classes_ids else self.anchor_strategy

            ids = train_df.loc[train_df[InputKeys.TARGET] == c, SpecialKeys.ID].tolist()  # type: ignore
