...
from src.enums import RunningStage, OutputKeys, SpecialKeys
...
class SaveOutputs(Callback):
    ...
    def on_epoch_end(self, model: _FabricModule, output: EPOCH_OUTPUT, metrics: METRIC, stage: RunningStage, **kwargs) -> None:
        ...
        instance_level_output = (
            pd.DataFrame(logits,  columns=[f"logit_{i}" for i in range(logits.shape[1])])
            .assign(**{SpecialKeys.ID: idx})
        )
        ...
        instance_level_output.to_parquet(path / f"{name}_instance_level.parquet", index=False)
        batch_level_output.to_parquet(path / f"{name}_batch_level.parquet", index=False)
        srsly.write_json(path / f"{name}_epoch_level.json", epoch_level_output)
